...
describe("update user identity test", () => {
  ...
  describe("User identity number", () => {
    describe("successful update", () => {
      test("should successfully update a user identity number", async () => {
        const response = await request(app)
          .put("/api/auth/add-identity-number")
          .set("Authorization", "Bearer mocked-token")
          .send({ identityNumber: "1234567890" })
          .expect(200);
    ...
    describe("Validation error", () => {
      test("should return 400 if no identity number is inputted", async () => {
        const response = await request(app)
          .put("/api/auth/add-identity-number")
          .set("Authorization", "Bearer mocked-token")
          .send({ identityNumber: "" })
    ...
    describe("Unauthorized request", () => {
      test("should return 401 for an unauthorized request", async () => {
        jest
          .spyOn(requiredAuthModule, "default")
          .mockImplementation(
            async (
    ...
    describe("Non-existing user", () => {
      test("should return 404 if there is no user", async () => {
        await User.deleteOne({ _id: userId });
