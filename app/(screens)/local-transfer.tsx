...
const LocalTransfer = () => {
  ...
  const handleSwipeSuccess = () => {
    router.push({
      pathname: ROUTES.LOCAL_BANK_DETAILS,
      params: {
        bankName: details.bankName,
        accountNumber: details.accountNumber,
        accountHolder: accountHolder,
        bankIcon:
          banks.find((bank) => bank.name === details.bankName)?.icon || "bank",
      },
    });
    
  };

  const findAccountHolder = (bankName: string, accountNumber: string) => {
    const bank = banks.find(
      (bank) =>
        bank.name.toLowerCase() === bankName.toLowerCase() &&
        bank.accountNumber === accountNumber
    );
    return bank ? bank.accountHolder : null;
  };

  const [isModalVisible, setModalVisible] = useState(false);
  const [searchBank, setSearchBank] = useState("");
  const [accountHolder, setAccountHolder] = useState<string | null>(null);
  const [isLoadingAccountHolder, setIsLoadingAccountHolder] = useState(false);
  ...
  React.useEffect(() => {
    if (details.bankName && details.accountNumber) {
      const holder = findAccountHolder(details.bankName, details.accountNumber);
      if (holder) {
        setIsLoadingAccountHolder(true);

        const timer = setTimeout(() => {
          setAccountHolder(holder);
          setIsLoadingAccountHolder(false);
        }, 5000);
    ...
    <ScrollView className="">
      <SafeAreaView className="flex-1">
        ...
        <CustomView className="pb-10">
          ...
          {accountHolder && !isLoadingAccountHolder && (
            <View className="flex flex-row items-center space-x-1 mt-3">
